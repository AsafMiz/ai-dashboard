...
|---|---|---|---|
| `datasetKey` | `string` | all (auto) | Auto-generated by the API — do not set manually |
...
Creates a new dashboard (metadata only, no datasets).
...
**Errors:** 400 (missing fields), 409 (key already exists)

---

### `GET /api/dashboards/[key]`

Returns dashboard metadata and a list of reports with URLs.

**Response (200):**
```json
{
  "dashboard": { "key": "example", "title": "...", "subtitle": "..." },
  "reports": [
    { "report_key": "sales-overview", "title": "Sales Overview", "url": "/dashboard/example/sales-overview" }
  ]
}
```
...
### `DELETE /api/dashboards/[key]`

Deletes a dashboard and all its reports, widgets, and datasets (via FK cascade).
...
Creates a report with inline widget data. Each widget contains its own `data` array. The API auto-generates datasets and assigns widget order by array position.
...
```json
{
  ...
  "widgets": [
    ...
    },
    {
      "type": "table",
      "title": "Sales Data",
      "config": { "columns": ["month", "revenue"], "columnLabels": { "month": "Month", "revenue": "Revenue" } },
      "data": [
        { "month": "Jan", "revenue": 145000 },
        { "month": "Feb", "revenue": 152000 }
      ]
...
| Field | Type | Required | Description |
|---|---|---|---|
| `dashboard_key` | `string` | Yes | Parent dashboard key (must exist) |
...
| `logo_url` | `string` | No | Logo image URL |
| `widgets` | `array` | Yes | Widget definitions with inline data |

**Widget fields:** `type` (string), `title` (string), `config` (object — do not include `datasetKey`, it's auto-generated), `data` (array of row objects)
...
```json
{
  ...
  "dashboard_key": "my-dashboard",
  "report_key": "sales-overview",
  "url": "/dashboard/my-dashboard/sales-overview"
}
```

**Errors:** 400 (missing fields), 404 (dashboard not found), 409 (report already exists)

---

### `DELETE /api/reports/[dashboardKey]/[reportKey]`

Deletes a report, its widgets, and associated datasets.

**Response (200):**
```json
{
  "message": "Report 'sales-overview' deleted successfully",
  "dashboard_key": "my-dashboard",
  "report_key": "sales-overview"
}
```
...
```
public/data/example/
├── dashboard.json              → metadata for POST /api/dashboards
└── reports/
    ├── sales-overview.json     → payload for POST /api/reports
...
Contains the dashboard metadata and a `reports` array listing report keys (used by the client to know which report files to load).
...
Each file is a self-contained payload for `POST /api/reports`. Widgets contain inline data:
  ...
  "widgets": [
    {
      "type": "area",
      "title": "Revenue Trend",
      "config": { "xKey": "month", "yKey": "revenue", "color": "#2563eb" },
      "data": [
        { "month": "Jan", "revenue": 145000 },
        { "month": "Feb", "revenue": 152000 }
      ]
...
2. **DELETE** `/api/dashboards/example` (ignore errors — may not exist)
3. **POST** `/api/dashboards` with dashboard metadata (key, title, subtitle, icon, logo_url)
4. **For each report key** in `dashboard.json → reports[]`:
   - **Fetch** `GET /data/example/reports/{report_key}.json`
   - **POST** `/api/reports` with report data + widgets (each widget has inline data)
5. **Navigate** to `/dashboard/example`

This populates all 4 database tables end-to-end:
- `dashboards` → 1 row
- `reports` → 7 rows
- `widgets` → ~30 rows
- `datasets` → ~30 rows (auto-generated, one per widget)
...
curl -X POST http://localhost:3000/api/dashboards \
  ...
  -d '{"key":"test","title":"Test Dashboard"}'

# 2. Create a report with inline data
curl -X POST http://localhost:3000/api/reports \
  -H "Content-Type: application/json" \
  -d '{"dashboard_key":"test","report_key":"overview","title":"Overview","widgets":[{"type":"bar","title":"Demo","config":{"xKey":"x","yKey":"y"},"data":[{"x":"A","y":10},{"x":"B","y":20}]}]}'

# 3. Get dashboard metadata
curl http://localhost:3000/api/dashboards/test

# 4. Delete dashboard (cascades to reports, widgets, datasets)
